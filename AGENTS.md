...
- 轨迹和命令提示使用原生覆盖层,不用 WebView;脚本引擎使用 rquickjs/QuickJS。
- 后端只存用户数据;更新和模板通过 GitHub 分发。生产部署由维护者使用 1Panel
  手动完成,交付物为 docker-compose。
...
- Bug 从日志和稳定复现开始;新增功能先确认术语、双平台完成定义和架构影响。
