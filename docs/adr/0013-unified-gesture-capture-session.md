...
- 是否允许进入活动捕获;
- 首个输入如何判定为普通触发键或边角序列首 token；两者都必须通过同一份已启用触发键校验;
- 从哪个配置作用域取得候选序列;
...
- 光标移动本身不能武装非空边角序列;
- 只有当前按下的是已启用的触发键时才进行边角准入；滚轮和左键只能作为已建立边角捕获后的后续输入。
...
3. 如果当前是已启用触发键的 `ButtonDown`，且没有普通捕获，先调用 `BoundaryAdmission`。
4. 边角位置存在匹配首 token 时，边角会话消费该触发键并开始统一会话；滚轮或左键不会单独创建边角会话。
...
- 光标移动到边缘不会武装非空边角序列；只有已启用触发键的首个按钮按下才进入边角捕获，滚轮和左键只能作为后续步骤。
