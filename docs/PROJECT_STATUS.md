...
| M8 打磨与发布         | 进行中;生产仓库坐标、prerelease 发布合同与快速入门已实现,真实发布和安装验收待执行 |
...
- 独立 `Mr-BeanSir/gesture-templates` 公共仓库尚未创建;已验证的种子位于 `distribution/gesture-templates`,正式发布前须按其 README 发布。自建 Server 不得代理该内容。
