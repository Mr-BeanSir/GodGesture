...
- 2026-08-20 GitHub Release 正文已改为发布时生成：GitHub 原生 `generate_release_notes` 只按合并 PR 生成，不能覆盖直接提交；v0.2.1 的 live body 已证明仓库无 PR 时会只有空的生成标记。`v0.2.2` 重跑期间 Windows hosted runner 在 debug Rust 编译阶段失联，实际 Rust 测试本身通过 `252 passed / 0 failed / 2 ignored`；Windows 发布 job 已改为带 30 分钟上限，并让 Rust library 测试使用 `--release`，以复用后续 Tauri release 构建产物，避免 debug/release 重复编译。tag job 现在由 `scripts/generate-release-notes.mjs` 查询上一个非 draft Release、比较 tag 提交、读取合并 PR，并按 `.github/release.yml` 的 12 类同时归类 PR title 与直接提交，输出固定说明、`Release Notes` 和 `Full Changelog`。因此 action 明确关闭原生生成以避免重复；v0.2.2 的 tag run #13 已在 `e38ddf1` 成功完成，但其 live body 仍是标题改名前的 `自动生成的 Release Notes`，当前标题修改将在下一次 tag 发布时生效。
...
- 2026-08-20 Release Notes 根因与回归验证：GitHub API 检查 v0.2.1 的 body 含原生生成标记但没有分类条目；`v0.1.0...v0.2.1` 有 196 个直接提交且仓库没有 PR。`scripts/generate-release-notes.mjs` 及其 4 个 Node 测试覆盖 12 类分组、PR title、直接提交、PR 提交去重、无历史 Release 回退链接和假 GitHub API/$GITHUB_OUTPUT 链路；本次标题改为 `Release Notes` 后，定向测试 `4/4`、发布校验 `42/42` 通过。v0.2.2 tag run #13 在 `e38ddf1` 成功生成双平台资产并更新 Release，但 live body 仍保留旧标题；真实 Windows/macOS 安装验收仍 pending。
