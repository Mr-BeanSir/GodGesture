...
- [x] Record the `macOS CI` workflow run and its `node-host-performance-macos-*` artifact. Run `30707285497` on commit `9b2ada6` completed the release gate with 10,000 ordered no-op events and 10,000 ordered representative host-call events; artifact `node-host-performance-macos-ARM64` has SHA-256 `1a80f9edb29d7c0bdddbc9a1bff6abdaf1bd1f531d21ada8232ee40f3838ba71`. The runner reported macOS 15.7.7 arm64, Node `v24.18.1`, cold start `257.88825 ms`, no-op p95/p99 `0.133084/0.231125 ms`, and host-call p95/p99 `0.403959/0.543875 ms`. This is CI evidence only and does not check the physical-Mac items below.
