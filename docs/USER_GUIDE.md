...
点击“录制新手势”后，按住触发键完成路径并释放。录制会持续到完成或显式取消；
若与现有手势冲突，界面会要求确认是否覆盖。

### 修饰与命令

手势期间可叠加滚轮、左键、中键、右键、X1 或 X2 修饰。启用“修饰触发时立即
执行”后，命令在修饰发生时执行，不等待触发键释放。
...
“触发角与摩擦边”页可为每个显示器角和屏幕边配置任意命令。鼠标按键按下时仍
会更新角/边状态，但不会分发命令。
