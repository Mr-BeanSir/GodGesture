...
边角起点后的零至 12 步有序输入。边缘带或精确角点周围的近角区域只有收到当前已启用触发键的首个按下事件后才进入边角捕获；滚轮和左键只能作为已建立捕获的后续步骤。首 token 有候选时继续匹配，未匹配时仍记录边角轨迹但不执行命令。未形成方向笔画时取消会重放已消费的原生输入；形成方向笔画后释放主触发键只结束捕获，不重放原生点击。方向笔画可作为后续步骤。空序列表示命中起点后立即执行。
...
只在区域准入、候选配置和覆盖层消息上保持差异。普通与边角准入共用已启用触发键判断；边或近角命中后由边角优先拥有输入,
不会因为首 token 没有候选而落入 `PathTracker`。
...
不随账户漫游的设置项:开机自启、托盘图标隐藏;Windows Desktop 始终以管理员身份启动,
不再提供管理员身份设置字段;其余设置全部参与同步。
